...
logo. `VIOLET.brand` is `--primary` from `web/src/index.css`; the rest of the
...
| `web/src/components/wisp-mark.tsx` | the reduction as a React component, for the app header and gallery |
...
The daemon serves exactly one file and no asset routes — `wispd/tests/web.test.ts`
