...
| `droid-unknown-model.stderr.txt` | `droid exec -o stream-json --skip-permissions-unsafe -m bogus-model "Reply with exactly the word: papaya"` (model catalog recaptured on droid 0.219.0; failure shape originally captured on 0.202.0) — this is the **stderr**; stdout was empty | 1 |
