...
is_wisp_checkout() {
  [ -f "$1/package.json" ] &&
    [ -f "$1/wispd/package.json" ] &&
    [ -f "$1/wispd/src/index.ts" ] &&
    [ -f "$1/wispd/scripts/wisp-dev" ] &&
    grep -Eq '"name"[[:space:]]*:[[:space:]]*"wispd"' "$1/wispd/package.json"
}
...
exec sh "$root/wispd/scripts/wisp-dev" "$@"
EOF
} > "$TMP"
