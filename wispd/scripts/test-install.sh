...
ARTIFACT="${WISP_ARTIFACT_PATH:-$ROOT/dist/release/v0.6.2/wisp-v0.6.2-linux-x86_64}"
...
docker --config "$CFG" run --rm --platform linux/amd64 \
  ...
  "$IMAGE" /bin/sh -eu -c '
    ...
    OCCUPIED_PID=$!
    OCCUPIED_READY=no
    # 100 attempts, 20 seconds. The daemon listens in well under a second; the
    # margin is for a slow runner, where a too-tight budget fails the whole run.
