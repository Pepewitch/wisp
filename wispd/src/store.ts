...
export function createTask(t: {
  ...
  effort?: string | null;
  /** run turns in the harness's faster lane; omitted = ordinary speed */
  fast?: boolean;
  ...
  return db.transaction((input: typeof t): Task => {
    ...
    db.run(
      `INSERT INTO task_contexts
         (task_id, n, harness, model, effort, fast, session_id, skills_json, created_at, updated_at)
       VALUES (?, 1, ?, ?, ?, ?, NULL, NULL, ?, ?)`,
      [input.id, input.harness, input.model, input.effort ?? null, fast, timestamp, timestamp],
    );
...
/**
 * The agent a task is asked to run: harness, model, and the two per-turn knobs
 * the harness may offer. One object rather than four positionals because two of
 * them are booleans, and adjacent booleans swap silently.
 */
...
export function switchTaskAgentBody(
  ...
  const { harness, model, effort } = agent;
  const fast = agent.fast ? 1 : 0;
  ...
  if (freshContext) {
    ...
    db.run(
      `INSERT INTO task_contexts
         (task_id, n, harness, model, effort, fast, session_id, skills_json, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, NULL, NULL, ?, ?)`,
      [taskId, contextN, harness, model, effort, fast, timestamp, timestamp],
    );
    db.run(
      ...
      // exists to avoid.
      `UPDATE tasks
       SET harness = ?, model = ?, effort = ?, fast = ?, context_n = ?, session_id = NULL, skills_json = NULL,
           context_tokens = NULL, updated_at = ?
       WHERE id = ?`,
      [harness, model, effort, fast, contextN, timestamp, taskId],
    );
  } else {
    db.run(
      `UPDATE task_contexts SET model = ?, effort = ?, fast = ?, updated_at = ? WHERE task_id = ? AND n = ?`,
      [model, effort, fast, timestamp, taskId, task.context_n],
    );
    db.run(
      `UPDATE tasks SET model = ?, effort = ?, fast = ?, updated_at = ? WHERE id = ?`,
      [model, effort, fast, timestamp, taskId],
    );
  }
 ...
 */
export function switchTaskAgent(taskId: string, agent: TaskAgentTarget, freshContext: boolean): Task {
  // db.transaction at CALL time: `db` is undefined until the daemon initializes it.
  return db.transaction(switchTaskAgentBody)(taskId, agent, freshContext);
}
...
export function createTurn(
  ...
  const requestedFast = (agent ? agent.fast : task?.fast === 1) ? 1 : 0;
  const res = db.run(
    `INSERT INTO turns
