...
const themeCss = await readFile(join(ROOT, "web/src/index.css"), "utf8");
const darkSurface = /--surface:\s*(#[\da-f]{6});/i.exec(themeCss)?.[1];
if (!darkSurface) throw new Error("web/src/index.css: no dark surface for theme-color");
const block = [
  START,
  `    <link rel="icon" type="image/svg+xml" href="${faviconDataUri(favicon)}" />`,
  `    <meta name="theme-color" content="${darkSurface}" />`,
  END,
].join("\n");
