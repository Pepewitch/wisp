...
**Wisp 0.5.2** adds task search (⌘F in a task, ⌘⇧F across tasks, and
`wisp search`), OpenCode as a fifth built-in harness, and a fix for
pull-request status drifting between the sidebar and task header. The 0.5
series brought the daemon and Desktop app together after
...
See [0.5.2 release notes](docs/v0.5/RELEASE-NOTES-0.5.2.md) and the
...
```sh
version=0.5.2 # replace with the current published release
...
artifact=dist/release/v0.5.2/wisp-v0.5.2-linux-x86_64
...
```sh
version=0.5.2 # replace with the installed version
curl --proto '=https' --tlsv1.2 -fsSL \
...
- [Desktop updates](docs/DESKTOP-UPDATES.md)
- [0.5.2 release notes](docs/v0.5/RELEASE-NOTES-0.5.2.md)
