...
**Current source version: `0.4.0-alpha.11`.** This is an experimental release
candidate, not production-ready software. Alpha.11 adds signed Desktop updates,
...
| Apple Silicon arm64, macOS 12.3+ configured minimum | Experimental alpha; alpha.11 publication is gated on Developer ID signing and notarization |
...
signed or notarized. The alpha.11 candidate cannot publish unless Developer ID
...
```sh
version=0.4.0-alpha.11 # replace with the current published alpha
...
artifact=dist/release/v0.4.0-alpha.11/wisp-v0.4.0-alpha.11-linux-x86_64
...
Homebrew bootstraps and repairs Wisp Desktop. Starting with alpha.11, the app's
...
the updater, so moving from it to alpha.11 requires
...
Open command or Privacy & Security approval. The alpha.11 tag workflow fails
...
```sh
version=0.4.0-alpha.11 # replace with the installed alpha
curl --proto '=https' --tlsv1.2 -fsSL \
