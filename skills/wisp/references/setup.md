...
is ad-hoc signed and not notarized. Starting with alpha.11, tag releases fail
closed unless Developer ID signing and notarization pass; do not bypass
Gatekeeper for an alpha.11 artifact that fails verification.
...
the older ad-hoc alpha.8; alpha.11 must instead pass its Developer ID and
...
self-update, so bootstrap alpha.11 with:
