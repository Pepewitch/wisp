...
Gatekeeper for an alpha.12 artifact that fails verification.
...
the older ad-hoc alpha.8; alpha.12 must instead pass its Developer ID and
...
self-update, so bootstrap alpha.12 with:
