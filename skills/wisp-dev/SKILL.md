...
| Prepare, publish, recover, or promote a versioned release or Homebrew update | [Releasing and publishing Wisp](references/releasing.md) — follow its **short path** step by step: `release:check`, `release:ready`, `release:closeout`; read further only for the reasoning or a manual fallback | Release scripts, promotion receipt, release notes, evaluator guide, and both repository diffs |
