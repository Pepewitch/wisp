...
alpha.12 application while preserving Desktop state. Alpha.15 carries the next
shared-interface fixes through that same channel and is intended to exercise a
second real in-app update, from alpha.13 to alpha.15.

Alpha.14 was never published: its fail-closed release workflow stopped before
signing or publication when the Linux installer test raced the readiness of its
occupied-port fixture. Alpha.15 includes the corrected readiness proof; no
alpha.14 release assets or Homebrew metadata exist.
...
Alpha.15 remains a release candidate until signing, notarization, stapling,
public-download verification, Homebrew publication, and the human-observed
alpha.13-to-alpha.15 updater journey have completed.
...
curl --proto '=https' --tlsv1.2 -fsSL \
  https://raw.githubusercontent.com/Pepewitch/wisp/v0.4.0-alpha.15/scripts/install.sh |
  sh
```
...
- Closed the Desktop release-channel audit loop so publication waits for the
  ...
  audit.
- Made the Linux installer release gate wait for its occupied-port fixture to
  answer an authenticated request before testing automatic port fallback.

## Known limits

- The alpha.13-to-alpha.15 updater path is not qualified until the published
...
- `wisp-v0.4.0-alpha.15-linux-x86_64`
- `release-manifest.json`
- `SHA256SUMS`
- `wisp-v0.4.0-alpha.15-darwin-arm64.tar.gz`
- `release-manifest-darwin-arm64.json`
- `SHA256SUMS-darwin-arm64`
- `wisp-desktop-v0.4.0-alpha.15-darwin-arm64.tar.gz`
- `wisp-desktop-v0.4.0-alpha.15-darwin-arm64.tar.gz.sig`
...
alpha.13-to-alpha.15 updater receipt remain pending until publication and
